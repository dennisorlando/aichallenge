...
class DocumentIndexer:
    ...
    def scan_now(self) -> Tuple[List[str], List[str]]:
        ...
        return self._scan()  # no lock here — _scan only locks around ChromaDB ops
    ...
    def _run(self):
        # No lock here — _scan only acquires it around ChromaDB ops
        self._scan()
    ...
    def _index_file(self, path: Path, path_str: str, file_hash: str) -> bool:
        ...
        The lock is held ONLY during the ChromaDB upsert, never during HTTP
        calls to Ollama — that's what caused the freeze.
        """
        try:
        ...
        # ── Embedding loop — NO lock held here ───────────────────────────────
        ...
        # ── ChromaDB upsert — lock only here ─────────────────────────────────
        batch = 50
        with self._lock:
